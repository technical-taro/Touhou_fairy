        ...
        if not enemy_paths[self.loaded_enemy][4]:#ここの処理は非紺珠伝型のフレーム処理
            if self.target_reached or enemy_paths[self.loaded_enemy][3][1] == 1:
                ...
                if self.target_x > 0:
                    ...
                    if ((tmr // 8) % self.flame_limit) != self.flame_limit and not self.flame_lock:
                        screen.blit(self.flame_img_list[1][(tmr // 8) % self.flame_limit], [self.x, self.y])
                    #elif ((tmr // 8) % self.flame_limit) == 3 and self.flame_lock:
                    elif ((tmr // 8) % self.flame_limit) == self.flame_limit:
                        screen.blit(self.flame_img_list[1][self.flame_limit], [self.x, self.y])
                        self.flame_lock = True
                        print("done")
                if self.target_x < 0:
                    if ((tmr // 8) % self.flame_limit - 1) != self.flame_limit and not self.flame_lock:
