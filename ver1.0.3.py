...
def main():
    global screen, enemy_paths, tmr, screen_size, keys, bullet
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, enemy_index):
        ...
        self.rect = Rect(0, 0, enemy_paths[enemy_index][2][0], enemy_paths[enemy_index][2][1])
    ...
    def built(self):
        ...
        pygame.draw.arc(screen, RED, [self.center_point[0] - self.radius_back, self.center_point[1] - self.radius_back, 2*self.radius_back, 2*self.radius_back], maths.radians(90), maths.radians(90+360*(self.current_hp/self.enemy_hp)), 7)
        pygame.draw.arc(screen, WHITE, [self.center_point[0] - self.radius, self.center_point[1] - self.radius, 2*self.radius, 2*self.radius], maths.radians(90), maths.radians(90+360*1), 3)
...
class Player(pygame.sprite.Sprite):
    def __init__(self, image_path):
        ...
        self.speed = 3
        self.sp_size = (32, 39)
        self.rect = Rect(0, 0, self.sp_size[0], self.sp_size[1])
...
class Bullet(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.x = 0
        self.y = 0
        self.rect = Rect(0, 0, 20, 59)
        self.moving = False
        self.split_point_x = [60, 35, 15, 0]
        self.split_point = [(176, (20, 59)), (113, (20, 59)), (52, (17, 59)), (0, (17, 53))]#1stは画像切り取りy軸位置、2ndは切り取り横幅、3rdは切り取り縦幅
        self.image = pygame.image.load("chara\\自機チルノ\\CirnoBullet.png")
        self.image_list, self.image_type = [], []
        for row in self.split_point_x:
            for size in self.split_point:
                self.surface = pygame.Surface(size[1])#pygame.SRCALPHA
                self.surface.blit(self.image, (0, 0), (row, size[0], size[1][0], size[1][1]))
                self.surface = self.surface.convert_alpha()
                self.image_type.append(self.surface)
            self.image_list.append(self.image_type)
            self.image_type = []
        self.image_list[2].append(self.image_list[3][0])
        del self.image_list[3]
    def shoot(self, x, y):
        self.x, self.y = x, y
        self.moving = True
        #if self.moving:
    ...
    def update(self):
        if self.y > 0:
            ...
            self.kill()
        screen.blit(self.image_list[2][4], [self.x, self.y])
